...
const backgroundProcesses = new Map<string, BgProcess>();
...
function createMcpServer() {
  ...
  server.registerTool(
    "run_command",
    {
      ...
      description:
        "Use this when the user explicitly asks to run a short-lived shell command on their local device. Use start_background for long-running processes. The server has no command allowlist, but it requires local terminal approval by default.",
      inputSchema: {
  ...
  server.registerTool(
    "start_background",
    {
      ...
      description:
        "Use this when the user explicitly asks to start a long-running local process without blocking the MCP request. Logs are captured in memory and can be fetched with get_background_logs.",
      inputSchema: {
        ...
        content: [
          {
            ...
            text:
              processes
                .map(
                  (entry) =>
                    `${entry.id}\t${entry.status}\texit=${entry.exitCode ?? "null"}\tpid=${entry.pid ?? "null"}\t${entry.command}`
                )
...
async function requestApproval(details: { action?: string; command: string; cwd: string; timeoutMs: number }) {
...
async function listDirectory(root: string, recursive: boolean, maxEntries: number) {
  const results: Array<{ path: string; type: "file" | "directory" | "other"; sizeBytes: number | null }> = [];
...
function textResult(operation: string, text: string, structuredContent: Record<string, unknown>) {
  return {
    structuredContent: {
      operation,
      ...structuredContent
...
function deniedResult(operation: string, targetPath: string) {
...
function pushBackgroundLog(entry: BgProcess, chunk: string) {
...
async function terminateBackgroundProcess(entry: BgProcess, signal: NodeJS.Signals) {
  if (!entry.proc.pid) {
    return;
  }
  ...
  if (process.platform === "win32") {
    try {
      ...
      pushBackgroundLog(entry, `[warn] taskkill failed: ${(error as Error).message}\n`);
    }
  }
...
function formatCommandResult(command: string, exitCode: number | null, stdout: string, stderr: string) {
  const output = [
    `Command: ${command}`,
...
function addCommonHeaders(req: Request, res: Response, next: () => void) {
...
function requireAuth(req: Request, res: Response, next: () => void) {
  ...
  res.status(401).json({
    error: "Unauthorized",
    message:
      "Set MCP_SHARED_SECRET on the server and connect with Authorization: Bearer <secret>, x-mcp-secret, or /mcp?token=<secret>."
  });
}
